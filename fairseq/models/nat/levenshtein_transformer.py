...
class LevenshteinTransformerModel(FairseqNATModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        self.y0_choice = getattr(args, "y0", 'blank')
        self.dual_training_for_deletion = getattr(args, "dual_training_for_deletion", False)
        self.dual_training_for_insertion = getattr(args, "dual_training_for_insertion", False)
        print('dual deletion:', self.dual_training_for_deletion)
        print('dual insertion:', self.dual_training_for_insertion)
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="dual training for deletion",
        )
        parser.add_argument(
            "--dual-training-for-insertion",
            action="store_true",
            help="dual training for insertion",
        )
        parser.add_argument(
            '--y0',
            default='blank',
            choices=['blank','source','target','self_gen']
        )
    ...
    def forward_ins(self, encoder_out, ins_input, tgt_tokens):
        if tgt_tokens.size(-1) < ins_input.size(-1):
            padding_mat = tgt_tokens.new_ones((tgt_tokens.size(0), ins_input.size(1) - tgt_tokens.size(1))) * self.pad
            tgt_tokens = torch.cat((tgt_tokens, padding_mat), dim=-1)
        if tgt_tokens.size(-1) > ins_input.size(-1):
            padding_mat = ins_input.new_ones((ins_input.size(0), tgt_tokens.size(-1) - ins_input.size(-1))) * self.pad
            ins_input = torch.cat((ins_input, padding_mat), dim=-1)
        # generate training labels for insertion
        masked_tgt_masks, masked_tgt_tokens, mask_ins_targets = _get_ins_targets(
            ins_input, tgt_tokens, self.pad, self.unk
        )
        mask_ins_targets = mask_ins_targets.clamp(min=0, max=255)  # for safe prediction
        mask_ins_masks = ins_input[:, 1:].ne(self.pad)
    ...
    def forward_del(self, encoder_out, del_input, tgt_tokens):
        word_del_targets = _get_del_targets(del_input, tgt_tokens, self.pad)
        word_del_out, _ = self.decoder.forward_word_del(
            normalize=False,
            prev_output_tokens=del_input,
            encoder_out=encoder_out,
        )
        word_del_masks = del_input.ne(self.pad)
        return {
            "word_del": {
                "out": word_del_out,
    ...
    def self_gen(self, encoder_out, tgt_tokens):

        pad = self.decoder.dictionary.pad()
        bos = self.decoder.dictionary.bos()
        eos = self.decoder.dictionary.eos()
        unk = self.decoder.dictionary.unk()
    ...
    def forward(
        ...
        y0 = None
        if self.y0_choice == 'blank':
            y0 = tgt_tokens.new_zeros(tgt_tokens.size(0), 2)
            y0[:, 0] = self.bos
            y0[:, 1] = self.eos
        elif self.y0_choice == 'source':
            y0 = src_tokens
        elif self.y0_choice == 'target':
            y0 = tgt_tokens
        ...
        # generate training labels for insertion
        ins_out = self.forward_ins(encoder_out, prev_output_tokens, tgt_tokens)
        word_ins_out = ins_out['word_ins']['out']
        masked_tgt_masks = ins_out['word_ins']['mask']
        ...
        del_out = self.forward_del(encoder_out, word_predictions, tgt_tokens)

        del_out_dual = None
        if self.dual_training_for_deletion:
            if y0 is None and self.y0_choice == 'self_gen':
                y0 = self.self_gen(encoder_out, tgt_tokens)
            assert y0 is not None
            del_input = y0
            del_out_dual = self.forward_del(encoder_out, del_input, tgt_tokens)
        ...
        if self.dual_training_for_insertion:
            ...
            ins_out_dual = {k + '_dual' : v for k,v in ins_out_dual.items()}
        
        ret = {}
        ret.update(ins_out)
        ret.update(del_out)
        if del_out_dual is not None:
            ret.update(del_out_dual)
        if ins_out_dual is not None:
            ret.update(ins_out_dual)
        return ret
...
@register_model_architecture("levenshtein_transformer", "levenshtein_transformer_to_bart_large")
def levenshtein_bart_large_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
